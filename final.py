...
mtcnn = MTCNN(margin=20, keep_all=False, device='cuda' if torch.cuda.is_available() else 'cpu')
...
def preprocess_image(image_path):
    ...
    aligned_face = aligned_face.permute(1, 2, 0).numpy()  # Convert to HWC format
    resized_image = cv2.resize(aligned_face, (input_shape[2], input_shape[3]))
...
def compare_faces(embedding1, embedding2, threshold=0.9):
    # Flatten embeddings to ensure they are 2D arrays
